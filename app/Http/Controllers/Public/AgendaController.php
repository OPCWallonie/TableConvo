...
class AgendaController extends Controller
{
    ...
    {
        $levelCode = $request->query('level') ?: null;

        // Code invalide ou absent → pas de filtre (fallback silencieux)
        $level = $levelCode
            ? Level::where('code', $levelCode)->where('is_active', true)->first()
            : null;
        ...
        return view('public.agenda.index', compact('tables', 'levels', 'levelCode'));
    }
}
